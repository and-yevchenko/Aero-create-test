    ...
    return (
        <div className='show-test-page'>
            <div className='show-test-page__container'>
                <Header>
                    <Link to='/create-test' className='show-test-page__button'>edit test</Link>
                </Header>
                {forms.length > 0 && 
                    <ol className='show-test-page__list'>
                        {!shuffleCards.shuffle ? 
                            forms.map((el) => (
                            <li key={el.id}>
                                <ReadyQuestion {...showResult} question={el.title} shuffle={shuffleCards.shuffle} el={el}/>
                            </li>))
                        :    
                            [...forms].sort(() => 0.5 - Math.random()).map((el) => (
                            <li key={el.id}> 
                                <ReadyQuestion {...showResult} question={el.title} shuffle={shuffleCards.shuffle} el={el}/>
                            </li>))
                        }
