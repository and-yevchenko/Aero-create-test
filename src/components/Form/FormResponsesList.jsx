...
export const FormResponsesList = ({ item, el }) => {
  ...
  useEffect(() => {
    if (props.openSettings) {
      props.setValue(
        'question',
        props.forms.find((form) => form.id === props.openSettings).title,
      );
      const allVariants = props.forms.find(
        (form) => form.id === props.openSettings,
      ).variants;
      if (allVariants.find((i) => i.id === item.id)?.answer) {
        props.setValue(
          `${item.id}`,
          `${allVariants.find((i) => i.id === item.id)?.answer}`,
        );
      } else {
        props.setValue(`${item.id}`, '');
      }
      inputCheckbox.current.checked = allVariants.find(
        (i) => i.id === item.id,
      )?.check;
      // props.unregister()
    }
  ...
  return (
    <li>
      <input
        ...
        type="text"
        className="list-inputs__variant"
        onInput={(e) =>
          !props.openSettings && props.onUpdateVariant(e, item.id)
        }
      ...
      {props.cardItem.variants.length > 1 && (
        <button onClick={(e) => props.onDeleteVariant(e, item.id)}>
          <X />
        </button>
      )}
      ...
      <label className="list-inputs__checks">
        <input
          ...
          name="checkbox"
          type="checkbox"
          className="list-inputs__checkbox"
          onClick={(e) =>
            !props.openSettings && props.onTrueVariant(e, item.id)
          }
        />
